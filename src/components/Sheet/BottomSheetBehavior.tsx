...
export default class BottomSheetBehavior extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    console.log('init: ', init, snapPoints);
    ...
    const tossForMaster =
      props.springConfig.hasOwnProperty('toss') &&
        ...
        : toss
    const destinationPoint = debug('destinationPoint', add(
      masterOffseted,
      multiply(tossForMaster, this.masterVelocity)
    ))

    /**
     * 1 大于snapPoints最后一个值 0 小于snapPoints最后一个值 、
     */
          ...
          >
            ...
            {this.props.contentPosition && (
              <Animated.Code
                exec={onChange(
                  this.Y,
                  set(this.props.contentPosition, sub(0, debug('contentPosition:', this.Y)))
                )}
              />
            )}
